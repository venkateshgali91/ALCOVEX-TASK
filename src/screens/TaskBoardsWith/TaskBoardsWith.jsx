...
const TaskBoardsWith = () => {
  const [showNewProjectPopup, setShowNewProjectPopup] = useState(false);
  const [taskBoards, setTaskBoards] = useState(['SBI Outsource', 'HPCL Project 1']);
    ...
    <div className="bg-[#ffffff] flex flex-row justify-center w-full">
      <div className="bg-[#ffffff] w-[1440px] h-[1024px] relative">
          ...
          <div className="inline-flex flex-col items-start gap-[24px] relative">
            ...
            <div className="flex flex-col w-[240px] items-start gap-[10px] px-[10px] py-0 relative flex-[0_0_auto]">
              ...
              {taskBoards.map((board, index) => (
                <Frame key={index} className="!self-stretch !flex-[0_0_auto] !w-full" stateProp="default" text={board} />
              ))}
            </div>
        ...
        <div className="absolute w-[1200px] h-[1024px] top-0 left-[240px]">
          {/* Lines */}
          {[304, 598, 892].map((left, index) => (
            <img key={index} className={`left-[${left}px] absolute w-[5px] h-[826px] top-[133px] object-cover`} alt="Line" src="/img/line-3.svg" />
          ))}
          {/* Status Badges and Components */}
          {[304, 612, 906].map((left, index) => (
            <div key={index} className={`inline-flex flex-col items-start gap-[20px] absolute top-[84px] left-[${left}px]`}>
              <div className="inline-flex flex-col items-start gap-[19px] relative flex-[0_0_auto]">
                {/* Adjust badgeType and text as needed */}
          ...
          {showNewProjectPopup && (
            <div className="absolute top-[100%] left-0">
              <NewProjectPopup onClose={handleCloseNewProjectPopup} onAddProject={handleAddProject} />
            </div>
          )}
        </div>
      </div>
